...
DEBUG = (os.getenv("DEBUG") is None) or (str(os.getenv("DEBUG")).lower() in ("1", "true", "yes", "y", "on"))
...
DATABASES = {
    'default': (
        # If MySQL env vars aren't provided, fall back to sqlite so the
        # project can run locally without additional infrastructure.
        (
            {
        ...
        if not (
            os.getenv("DB_NAME") and os.getenv("DB_USER") and os.getenv("DB_PASSWORD")
        )
        else
        {
