...
class SheetListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Sheet
        fields = [
            'id', 'name', 'progress', 'slug', 'thumbnail',
            'description', 'sheet_type', 'is_sequential',
            'total_questions', 'is_enabled'
        ]

    def get_progress(self, obj):
        # Use precomputed progress_map from context (no DB hit)
...
class QuestionListSerializer(serializers.ModelSerializer):
    ...
    tag_list = serializers.ListField(
        child=serializers.CharField(), source='tags.all',
    ...
    class Meta:
        model = Question
        fields = [
            'id', 'title', 'slug', 'difficulty_level', 'difficulty_color',
            'youtube_link', 'position', 'status', 'color', 'is_enabled', 'tag_list'
        ]
...
class MCQQuestionListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MCQQuestion
        fields = [
            'id', 'question_text', 'slug', 'difficulty_level',
            'difficulty_color', 'status', 'is_enabled', 'tag_list'
        ]
