...
class BatchDetailView(APIView):
    ...
    def get(self, request, slug):
        ...
        active_sheets = Sheet.objects.filter(active_conditions).distinct().order_by('id')
        ...
        # 5. Precompute per-sheet progress (avoids N+1 queries in serializer)
        progress_map = {}
        for sheet in active_sheets:
            total = sheet.get_total_questions()
            solved = sheet.get_solved_questions(student)
            progress_map[sheet.id] = int((solved / total * 100) if total > 0 else 0)

        # 6. Serialize Sheets — pass progress_map via context
        sheet_data = SheetListSerializer(
            active_sheets,
            many=True,
            context={
                'request': request,
        ...
        if pod:
            ...
            # Check if solved (MCQ) if not already solved
            if not is_solved:
