...
class CourseAdmin(admin.ModelAdmin):
    list_display = ['name', 'instructor', 'is_active', 'assignment_count', 'created_at']
    list_filter = ['is_active', 'created_at', 'instructor']
    search_fields = ['name', 'description', 'instructor__username', 'instructor__first_name', 'instructor__last_name']
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'slug', 'instructor', 'description')
        }),
        ('Media', {
            'fields': ('thumbnail',)
        }),
    ...
    def assignment_count(self, obj):
        """Display count of assignments for this course"""
        ...
        if count > 0:
            url = reverse('admin:student_assignment_changelist')
            return format_html(
                '<a href="{}?content_type__id__exact={}&object_id__exact={}">{} assignments</a>',
                url, course_type.id, obj.id, count
            )
...
class AssignmentSubmissionAdmin(admin.ModelAdmin):
    list_display = [
        'get_assignment_title', 'get_student_name', 'status', 
    ...
    fieldsets = (
        ...
        ('Submission Content', {
            'fields': ('submission_preview', 'submission_text', 'submission_code', 
                      'submission_file', 'submission_image', 'submission_link')
        }),
        ('Grading', {
            'fields': ('status', 'score', 'feedback')
        }),
        ('Late Submission Info', {
            'fields': ('late_status', 'penalty_display', 'final_score_display'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
